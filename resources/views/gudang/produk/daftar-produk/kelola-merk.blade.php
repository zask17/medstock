...
@section('content')
    <div class="space-y-6" x-data="{ openModalMerk: false }" @keydown.escape.window="openModalMerk = false">
        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-xs p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <button @click="openModalMerk = true" class="bg-[#149387] hover:bg-[#117c71] text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-xs flex items-center gap-2 transition-all cursor-pointer">
        ...
        <div x-show="openModalMerk" 
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-black/50 backdrop-blur-xs transition-opacity" @click="openModalMerk = false"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
                <div class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full p-6 border border-gray-100 relative z-10" @click.outside="openModalMerk = false">
                    <h3 class="text-lg font-bold text-gray-800 mb-4">Tambah Merk Baru (Add New Brand)</h3>
                    <form @submit.prevent="openModalMerk = false" class="space-y-4 text-xs font-semibold">
                        <div>
                        ...
                        <div class="pt-4 border-t border-gray-100 flex justify-end gap-2">
                            <button type="button" @click="openModalMerk = false" class="bg-gray-100 hover:bg-gray-200 text-gray-600 px-5 py-2.5 rounded-xl font-bold cursor-pointer">Batal</button>
