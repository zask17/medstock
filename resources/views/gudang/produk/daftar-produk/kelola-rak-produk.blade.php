...
@section('title', 'Kelola Rak Produk')

@section('content')
    <div class="space-y-6" x-data="{ openModalRak: false }" @keydown.escape.window="openModalRak = false">
        <div class="flex flex-wrap items-center gap-2 border-b border-gray-100 pb-3">
            ...
            <a href="{{ route('gudang.produk.kelola-rak-produk') }}" class="bg-[#149387] text-white px-4 py-2 rounded-xl text-xs font-bold shadow-xs">Kelola Rak Produk</a>
        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-xs p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div class="flex items-center gap-2 flex-1 max-w-md">
                    <input type="text" placeholder="Cari rak produk..." class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#149387]">
                ...
                <button @click="openModalRak = true" class="bg-[#149387] hover:bg-[#117c71] text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-xs flex items-center gap-2 transition-all cursor-pointer">
                    <i class="fa-solid fa-plus text-xs"></i> Tambah Rak Produk
                </button>
            </div>
                ...
                <table class="w-full text-left border-collapse text-sm min-w-[700px]">
                    <thead>
                        <tr class="border-b border-gray-100 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                            ...
                            <th class="pb-4 px-4">Nama Rak Produk</th>
                    ...
                    <tbody class="divide-y divide-gray-50 font-medium text-gray-600">
                        @php
                            $racks = ['Square', 'Beximco', 'Rak 1', 'Rak 2'];
                        @endphp
                        @foreach ($racks as $index => $r)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="py-4 px-4 font-bold text-gray-800">{{ $r }}</td>
        ...
        <div x-show="openModalRak" 
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-black/50 backdrop-blur-xs transition-opacity" @click="openModalRak = false"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
                <div class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full p-6 border border-gray-100 relative z-10" @click.outside="openModalRak = false">
                    <h3 class="text-lg font-bold text-gray-800 mb-4">Tambah Rak Produk Baru (Add New Product Rack)</h3>
                    <form @submit.prevent="openModalRak = false" class="space-y-4 text-xs font-semibold">
                        <div>
                            <label class="block text-gray-500 mb-1.5">Nama Rak Produk (Product Rack Name)</label>
                            <select class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 focus:outline-none focus:border-[#149387] cursor-pointer">
                                <option value="">Pilih / Tulis Nama Rak</option>
                                <option value="Rak A1">Rak A1</option>
                                <option value="Rak B2">Rak B2</option>
                            </select>
                        </div>
                        <div class="pt-4 border-t border-gray-100 flex justify-end gap-2">
                            <button type="button" @click="openModalRak = false" class="bg-gray-100 hover:bg-gray-200 text-gray-600 px-5 py-2.5 rounded-xl font-bold cursor-pointer">Batal</button>
