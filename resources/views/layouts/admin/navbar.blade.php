<header class="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-20">
    <div class="flex items-center space-x-4">
        ...
        <div class="hidden sm:flex items-center bg-gray-50 rounded-full px-4 py-2 w-64 md:w-96">
            ...
            <input type="text" placeholder="Search..." class="bg-transparent border-none focus:outline-none text-sm w-full">
        </div>
    </div>
    ...
    <div class="flex items-center space-x-4 sm:space-x-6">
        ...
        <div class="flex items-center space-x-3 sm:border-l sm:pl-6">
            <div class="text-right hidden md:block">
                <p class="text-sm font-bold text-gray-800">{{ session('user_nama', 'Thomas F') }}</p>
                <p class="text-xs text-gray-400">{{ session('user_role', 'Admin') }}</p>
            </div>
            <img src="https://ui-avatars.com/api/?name={{ session('user_nama', 'Thomas F') }}&background=149387&color=fff" class="w-9 h-9 sm:w-10 sm:h-10 rounded-full border-2 border-[#149387]">
        </div>
    </div>
</header>